...
// par[5] = Linear Background Intercept, par[6] = Linear Background Slope
...
Double_t arg = 0;
 ...
 //Low Shoulder Exponential
  Double_t LEXP = par[2]*TMath::Exp(0.5*par[3]*par[3] + par[3]*arg);
 
//Gaussian
Double_t Gauss = par[2]*TMath::Exp(-0.5*arg*arg);

//High Shoulder Exponential
Double_t HEXP = par[2]*TMath::Exp(0.5*par[4]*par[4] - par[4]*arg);
