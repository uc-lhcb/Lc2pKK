...
void DocaEndVSB::SlaveBegin(TTree * /*tree*/)
{
   ...
   ENDVDOCAEvents->GetZaxis()->SetTitle("Entries");

   ENDVDOCASignal = new TH2D("", "", 100, 0., 0.2, 100, 0., 1.5);
   ENDVDOCASignal->GetZaxis()->SetTitle("Entries");

   ENDVDOCABackground = new TH2D("", "", 100, 0., 0.2, 100, 0., 1.5);
   ENDVDOCABackground->GetZaxis()->SetTitle("Entries");

   ENDVDOCASignalEstimate = new TH2D("", "", 100, 0., 0.2, 100, 0., 1.5);
   ENDVDOCASignalEstimate->GetZaxis()->SetTitle("Entries");
...
&& TMath::Log10(*Lc_ENDVERTEX_CHI2) < 1.2
 ...
 if(PIDBSubbedCuts && DalitzCuts && MassRange){
...
f1->Draw("SAME");
f2->Draw("SAME");
lt->DrawLatexNDC(0.75, 0.10, "LHCb Preliminary");
...
ENDVDOCASignalEstimate->Draw("CONTZ");
ENDVDOCASignalEstimate->GetZaxis()->SetTitle("(Signal Events)/(Background Events)");
