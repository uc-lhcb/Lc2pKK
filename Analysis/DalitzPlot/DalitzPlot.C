...
void DalitzPlot::SlaveBegin(TTree * /*tree*/)
{
   TString option = GetOption();
      ...
      DalitzPlot1->GetZaxis()->SetTitle("Events");
      ...
      PIDDalitzMass->GetXaxis()->SetTitle("m(pKK) [MeV]");
      PIDDalitzMass->GetYaxis()->SetTitle("Events / 0.5 MeV");

      File = new TFile("DalitzPlot.root", "RECREATE");
       gFile = File;
...
gStyle->SetOptStat(0);
DalitzSignal->GetYaxis()->SetTitle("Events / 0.002 GeV^{2}");
